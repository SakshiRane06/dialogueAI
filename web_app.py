...
class WebDialogueGenerator:
    ...
    def generate_dialogue(self, user_goal: str, tone: str = "conversational", difficulty: str = "intermediate") -> tuple:
        ...
        try:
            ...
            # Try Google API first, fallback to mock dialogue
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        try:
            ...
            # Process document
            content, stats, success = dialogue_gen.process_document(file_path)
...
def api_status():
    ...
    return jsonify({
        'status': 'running',
        'openai_available': openai_key,
        'google_available': google_key,
        'google_ai_installed': GOOGLE_AI_AVAILABLE,
        'embedding_fallback': 'SentenceTransformers' if not openai_key else 'OpenAI'
    })
